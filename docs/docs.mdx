...
Pass `onScrollStart` and `onScrollEnd` to react to interaction boundaries — useful for analytics, haptic feedback, or coordinating multiple pickers in a group. A single continuous interaction (drag → momentum, or wheel + animated snap) only fires the pair once: `onScrollStart` triggers when the picker leaves the idle state, `onScrollEnd` fires when every internal animation, momentum, drag and wheel debounce has settled.

<Demo data={demos.pickerScrollEvents} />
...
<Demo data={demos.pickerColorPalette} />
...
<Demo data={demos.pickerAlarm} />

## Example: Slot Machine

As funny as it may sound, the `Picker` component can be used to create a slot machine effect — three reels spin at different speeds, stop sequentially, and emit a tactile pulse on every reel landing (and a longer pattern on a jackpot).
